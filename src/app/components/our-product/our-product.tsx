import Image from "next/image"
import { BsCartDash } from "react-icons/bs";
export default function OurProduct() {
    return (
        <div className="overflow-x-hidden">




            {/* our product */}


                <h1 className="text-center font-bold mt-[55px] mb-[55px] sm:text-[20px] lg:text-[25px]">Our Product</h1>

            <div className=" flex flex-col items-center">

                <div className="lg:flex  ">
                    <div className="md:flex md:gap-10">
                        <div>
                            <Image className="lg:w-[200px]  sm:w-[250px] " src="/image/Image.png" alt="logo" width={212} height={312} />
                            <div className="flex">
                                <p className="text-[#007580] sm:text-[20px] lg:font-bold lg:text-[13px] sm:mb-[5px]"> Library Stool Chair</p>
                                <BsCartDash  className="w-[33px] h-[33px] bg-[#029FAE] rounded-[5px] text-white relative top-[11px] left-[40px]"/>
                            </div>
                            <h1 className="relative bottom-[10px] sm:text-[19px] lg:font-semibold lg:relative lg:bottom-[15px]">$20</h1>
                            <h1 className="lg:relative lg:bottom-[250px]  sm:relative sm:bottom-[300px] relative bottom-[260px] left-[15px] bg-[#01AD5A] w-[49px] h-[26px] text-center text-white rounded-[4px] ">New</h1>
                        </div>




                        <div>

                            <Image className="  lg:w-[200px] sm:w-[250px] lg:mr-[45px] " src="/image/Image (1).png" alt="logo" width={212} height={312} />
                            <div className="flex">
                                <p className="text-[#272343]  sm:text-[20px] lg:font-bold lg:text-[13px] sm:mb-[5px]">Library Stool Chair</p>
                                <BsCartDash  className="w-[33px] h-[33px] bg-[#F0F2F3] rounded-[5px] relative top-[11px] left-[40px]"/>
                            </div>
                            <div className="flex gap-2">
                                <h1 className="relative bottom-[10px] sm:text-[19px] lg:font-semibold  lg:relative lg:bottom-[15px]">$20</h1>
                                <p className="line-through relative bottom-[10px] text-[#9A9CAA] lg:font-semibold  lg:relative lg:bottom-[15px]">$39</p>
                            </div>
                            <h1 className=" lg:relative lg:bottom-[250px]  relative bottom-[260px] sm:relative sm:bottom-[300px]  left-[15px] bg-[#F5813F] w-[49px] h-[26px] text-center text-white rounded-[4px] ">Sales</h1>
                        </div>
                    </div>



                    <div className="md:flex md:gap-10">
                        <div>
                            <Image className="  lg:w-[200px] sm:w-[250px] " src="/image/Frame.png" alt="logo" width={212} height={312} />
                            <div className="flex">
                                <p className="text-[#272343]  sm:text-[20px] lg:font-bold lg:text-[13px] sm:mb-[5px]">Library Stool Chair</p>
                                <BsCartDash  className="w-[33px] h-[33px] bg-[#F0F2F3] rounded-[5px] relative top-[11px] left-[40px]"/>
                            </div>
                            <h1 className="relative bottom-[10px] sm:text-[19px] lg:font-semibold  lg:relative lg:bottom-[15px]">$20</h1>

                        </div>



                        <div>
                            <Image className="  lg:w-[200px] sm:w-[250px] " src="/image/01.png" alt="logo" width={212} height={312} />
                            <div className="flex">
                                <p className="text-[#272343]  sm:text-[20px] lg:font-bold lg:text-[13px] sm:mb-[5px]">Library Stool Chair</p>
                                <BsCartDash  className="w-[33px] h-[33px] bg-[#F0F2F3] rounded-[5px] relative top-[11px] left-[40px]"/>
                            </div>
                            <h1 className="relative bottom-[10px] sm:text-[19px] lg:font-semibold  lg:relative lg:bottom-[15px]">$20</h1>







                        </div>

                    </div>
                </div>






















                <div className=" flex flex-col items-center">
                    <div className="lg:flex">

                        <div className="md:flex md:gap-10">

                            <div>
                                <Image className="  lg:w-[200px] sm:w-[250px] " src="/image/Frame (3).png" alt="logo" width={212} height={312} />
                                <div className="flex">
                                    <p className="text-[#272343]  sm:text-[20px] lg:font-bold lg:text-[13px] sm:mb-[5px]">Library Stool Chair</p>
                                    <BsCartDash  className="w-[33px] h-[33px] bg-[#F0F2F3] rounded-[5px] relative top-[11px] left-[40px]"/>
                                </div>
                                <h1 className="relative bottom-[10px] sm:text-[19px] lg:font-semibold lg:relative lg:bottom-[15px]">$20</h1>
                                <h1 className="  lg:relative lg:bottom-[250px] relative bottom-[260px] sm:bottom-[300px] left-[15px] bg-[#01AD5A] w-[49px] h-[26px] text-center text-white rounded-[4px] ">New</h1>
                            </div>




                            <div>
                                <Image className=" lg:w-[200px]  lg:mr-[45px] sm:w-[250px] " src="/image/card.png" alt="logo" width={212} height={312} />
                                <div className="flex">
                                    <p className="text-[#272343]  sm:text-[20px] lg:font-bold lg:text-[13px] sm:mb-[5px]">Library Stool Chair</p>
                                    <BsCartDash  className="w-[33px] h-[33px] bg-[#F0F2F3] rounded-[5px] relative top-[11px] left-[40px]"/>
                                </div>
                                <div className="flex gap-2">
                                    <h1 className="relative bottom-[10px] sm:text-[19px] lg:font-semibold lg:relative lg:bottom-[15px]">$20</h1>
                                    <p className="line-through relative bottom-[10px] text-[#9A9CAA] lg:font-semibold lg:relative lg:bottom-[15px]">$39</p>
                                </div>
                                <h1 className="  lg:relative lg:bottom-[250px]   relative bottom-[260px] left-[15px] sm:relative sm:bottom-[300px] bg-[#F5813F] w-[49px] h-[26px] text-center text-white rounded-[4px] ">Sales</h1>
                            </div>
                        </div>


                        <div className="md:flex md:gap-10">
                            <div>
                                <Image className=" lg:w-[200px] sm:w-[250px] " src="/image/Image (13).png" alt="logo" width={212} height={312} />
                                <div className="flex">
                                    <p className="text-[#272343]  sm:text-[20px] lg:font-bold lg:text-[13px] sm:mb-[5px]">Library Stool Chair</p>
                                    <BsCartDash  className="w-[33px] h-[33px] bg-[#F0F2F3] rounded-[5px] relative top-[11px] left-[40px]"/>
                                </div>
                                <h1 className="relative bottom-[10px] sm:text-[19px] lg:font-semibold lg:relative lg:bottom-[15px]">$20</h1>

                            </div>



                            <div>
                                <Image className="  lg:w-[200px] sm:w-[250px] " src="/image/Image (5).png" alt="logo" width={212} height={312} />
                                <div className="flex">
                                    <p className="text-[#272343]  sm:text-[20px] lg:font-bold lg:text-[13px] sm:mb-[5px]">Library Stool Chair</p>
                                    <BsCartDash className="w-[33px] h-[33px] bg-[#F0F2F3] rounded-[5px] relative top-[11px] left-[40px]"/>
                                </div>
                                <h1 className="relative bottom-[10px] sm:text-[19px] lg:font-semibold lg:relative lg:bottom-[15px]">$20</h1>

                            </div>
                        </div>
                    </div>
                </div>
            </div>





        </div>

    )
}
